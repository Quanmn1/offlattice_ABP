...
while i < len(tests):
    ...
    i += num_params
    data = np.loadtxt(test_name+'_sigma_data', skiprows=1)
    rhos = data[:,0]
    sigmas = data[:,1]
    marker = '.'
    indices = rhos<1.5
    ax.plot(rhos[indices], sigmas[indices], label=fr"$dt = {dt}$", marker=marker, ls='', ms=10, c=color)
    # popt, pcov = optimize.curve_fit(sigma_fit, rhos, sigmas, p0=(0.5,4, 2e-9, 10))
    # # print(popt)
    # ax.plot(rho_dense, sigma_fit(rho_dense, *popt), c=color)
...
ax.legend()
plt.savefig('pfap_harmonic_pressures_largeeps.png',  dpi=300, bbox_inches='tight')
