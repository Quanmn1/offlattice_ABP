...
def plot_arrow(ax, x, y, theta, scale=0.5):
    dx = np.cos(theta) * scale
    dy = np.sin(theta) * scale
    ax.arrow(x-dx/2, y-dy/2, dx, dy, width=0.05, color='C0')
...
with open(data_file, 'r') as f:
    ...
    for line in f:
        ...
        ax.set_title(f'N={N}, v={v}, Dr={Dr}, t={t}')
        ax.set_xlabel('x')
        ax.set_ylabel('y')
        ...
        plt.savefig(name + f't={t}.png', dpi=300, bbox_inches='tight')
        plt.close()
