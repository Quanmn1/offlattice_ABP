...
def analyze_sigma(test_name, mode, vars, num_segments, walls=False, data="sigmaIK"):
    ...
    if mode == "pfap":
        ...
        box_size = int(np.ceil(params['r_max_pfap']))
    else:
        file = test_name + f'_{rhos[0]:.0f}'
        params, N = read_param(file + '_param')
        box_size = params['r_max_qsap']

    density_box_size = params['density_box_size']*box_size
    Lx = params['Lx']
    ...
    if walls:
        bulk_left = int(Lx*3/10)
        bulk_right = int(Lx*7/10)
    else:
        bulk_left = 0
        bulk_right = int(Lx)
    ...
    sigmas_avg = np.zeros_like(rhos)
    std_sigma = np.zeros_like(rhos)
    rho_bulk = np.zeros_like(rhos)
    std_density = np.zeros_like(rhos)
    L = Lx * final_time // time_step # size of sigma array
    N = Lx * final_time // density_box_size // time_step # size of density array
    ...
    for (ind,rho) in enumerate(rhos):
        ...
        try:
            sigma=np.loadtxt(sigmafile)[Lx//box_size:,:] # discard the measurement at the start, otherwise we can't divide up into even intervals
        except OSError:
        ...
        for i in range(num_segments):
            if L*(i+1)//num_segments <= sigma.shape[0]:
        ...
        # take avg and std.
        sigma_eachvar, density_eachvar = nonzero(sigmas[ind,:], rhos_bulk[ind,:])
        sigmas_avg[ind] = np.mean(sigma_eachvar[equi_index:], axis=-1)
        std_sigma[ind] = np.std(sigma_eachvar[equi_index:], axis=-1)/np.sqrt(len(sigma_eachvar)-equi_index-1)
        std_density[ind] = np.std(density_eachvar[equi_index:], axis=-1)/np.sqrt(len(density_eachvar)-equi_index-1)
        rho_bulk[ind] = np.mean(density_eachvar[equi_index:], axis=-1)
    ...
    markersize=7
    capthick=1.5
    ax.errorbar(rho_bulk, sigmas_avg, xerr=std_density, yerr=std_sigma, ls='', marker='.', ms=4, label=f"Bulk {data} pressure")
    ...
    # WALLS: code not updated yet
    if walls:
        space_between_plotted_bars = 0.01
        std_left= np.std(lefts[:,equi_index:], axis=-1)
        std_right = np.std(rights[:,equi_index:], axis=-1)
    ...
    plt.savefig(f'{test_name}_{plot_name}.png',  dpi=300, bbox_inches='tight')
